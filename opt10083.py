#opt10083 : 주식월봉차트조회요청
#부가설명 : 입력한 날짜 기준으로 월봉에 대하여 과거의 시가 저가 고가 종가를 받아올 수 있음. 코드상으로 보면 주봉차트조회요청과 별반 다를게 없음.
...
class Program(QMainWindow):
    ...
    def requestTrData(self, c):
            self.kiwoom.dynamicCall("SetInputValue(QString, QString)", "종목코드", "005930")    # 종목코드
            ...
            self.kiwoom.dynamicCall("CommRqData(QString, QString, int, QString)", "opt10083_req", "opt10083", c, "0766") # 사용사요청명칭 / 요청함수 / 초기조회:0, 연속조회:2 / 화면번호

    def receiveTrData(self, screen_no, rqname, trcode, recordname, prev_next, data_len, err_code, msg1, msg2):
        if rqname == "opt10083_req":
            print(prev_next)
            for i in range(300): # 한번 요청에 240개의 정보가 들어오는데 혹시몰라서 큰 값으로 잡음.
